...
@section('body')
    ...
    <course-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/courses') }}'" inline-template>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/courses/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.course.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'class_id'">
                                            {{ trans('admin.course.columns.class_id') }}</th>
                                        <th is='sortable' :column="'name'">{{ trans('admin.course.columns.name') }}</th>
                                        <th is='sortable' :column="'strength'">
                                            {{ trans('admin.course.columns.strength') }}</th>
                                        <th is='sortable' :column="'level'">{{ trans('admin.course.columns.level') }}
                                        </th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/courses')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/courses/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td>@{{ item.name }}</td>
                                        <td v-if="item.strength == 1">@{{ item.strength }} Hour</td>
                                        <td v-else>@{{ item.strength }} Hours</td>
                                        <td v-if="item.level == 0">Beginner</td>
                                        <td v-if="item.level == 1">Intermediate</td>
                                        <td v-if="item.level == 2">Advanced</td>
                                        <td v-if="item.level == 3">Expert</td>
                                        <td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/courses/create') }}"
